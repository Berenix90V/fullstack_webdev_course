...
describe('creation of a blog', () => {
    test('it is possible to add a blog', async () => {
        ...
        const savedBlog = blogsAtTheEnd.find(
            (blog) => blog.id.toString() === savedBlogID,
        )
...
describe('deletion of a blog', () => {
    ...
    test('fails with status code 400 if malformed token', async () => {
        const blogsAtStart = await helper.blogsInDb()
        const blogToDelete = blogsAtStart[0]
        const invalidToken = 'abc'
        await api
...
describe('update of a blog', () => {
    ...
    test('fails with status code 400 if id is not existent', async () => {
        ...
        await api.put(`/api/blogs/${blogIdNotValid}`).send(newBlog).expect(400)
    })
})
